...
static void ui_draw_sidebar_battery_text(UIState *s) {
  const int battery_img_w = 120;
  const int battery_img_x = 140;
...
static void ui_draw_sidebar_metric(UIState *s, const char* label_str, const char* value_str, const int severity, const int y_offset, const char* message_str) {
  ...
  if (!message_str) {
    ...
    nvgTextAlign(s->vg, NVG_ALIGN_LEFT | NVG_ALIGN_MIDDLE);
    nvgTextBox(s->vg, metric_x + 50, metric_y + 35, metric_w - 60, value_str, NULL);

    nvgFillColor(s->vg, COLOR_WHITE);
    nvgFontSize(s->vg, 48);
    nvgFontFaceId(s->vg, s->font_sans_regular);
    nvgTextAlign(s->vg, NVG_ALIGN_LEFT | NVG_ALIGN_MIDDLE);
    nvgTextBox(s->vg, metric_x + 50, metric_y + 30 + 66, metric_w - 60, label_str, NULL);
  } else {
    nvgFillColor(s->vg, COLOR_WHITE);
    nvgFontSize(s->vg, 48);
    nvgFontFaceId(s->vg, s->font_sans_bold);
...
static void ui_draw_sidebar_temp_metric(UIState *s) {
  ...
  const int temp_y_offset = 20;
  std::string temp_val = std::to_string((int)s->scene.thermal.getAmbient()) + "°C";
  ui_draw_sidebar_metric(s, "이온온도", temp_val.c_str(), temp_severity_map[s->scene.thermal.getThermalStatus()], temp_y_offset, NULL);
