...
param = ["accelerationMode", "Kp", "Ki", "Kf", "steerRatio", "steerRateCost", "steerActuatorDelay", \
          "steerLimitTimer", "sR_BP0", "sR_BP1", "sR_boost", "sR_time", "slowOnCurves"]
...
while True:
  print ("")
  print (print_letters(param[j][0:16]))
  print ("")
  print (print_letters(kegman.conf[param[j]]))
  print ("")
  print ("1,2,3,4,5 to incr 0.1,0.01,0.001,0.0001,0.00001")
  print ("q,w,e,r,t to decr 0.1,0.01,0.001,0.0001,0.00001")
  ...
  if (char == "r"):
    kegman.conf[param[j]] = str(round((float(kegman.conf[param[j]]) - 0.0001),5))
    write_json = True
  ...
  if float(kegman.conf['steerLimitTimer']) < 0 and float(kegman.conf['steerLimitTimer']) != -1:
    kegman.conf['steerLimitTimer'] = "0"

  if float(kegman.conf['steerLimitTimer']) > 4:
    kegman.conf['steerLimitTimer'] = "4"
